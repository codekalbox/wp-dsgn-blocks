...
if ( ! class_exists( 'PBG_WebFont_Loader' ) ) {
	/**
	 * Download webfonts locally.
	 */
	class PBG_WebFont_Loader {
		...
		public function __construct( $url = '' ) {
			...
			add_action( 'pbg_delete_fonts_folder', array( $this, 'pbg_delete_fonts_folder' ) );
		}
		...
		public function get_local_files_from_css() {
			...
			$stored     = get_site_option( 'pbg_downloaded_font_files', array() );
			...
			if ( $change ) {
				...
				update_site_option( 'pbg_downloaded_font_files', $stored );
			}
		...
		public function preload_local_fonts() {
			...
			update_site_option( 'pbg_local_font_files', $local_font );
		...
		public function get_base_path() {
			if ( ! $this->base_path ) {
				$this->base_path = apply_filters( 'pbg_get_local_fonts_base_path', $this->get_filesystem()->wp_content_dir() . '/uploads/premium-blocks-for-gutenberg' );
			}
		...
		public function get_base_url() {
			if ( ! $this->base_url ) {
				$this->base_url = apply_filters( 'pbg_get_local_fonts_base_url', content_url() . '/uploads/premium-blocks-for-gutenberg' );
			}
		...
		public function get_subfolder_name() {
			if ( ! $this->subfolder_name ) {
				$this->subfolder_name = apply_filters( 'pbg_get_local_fonts_subfolder_name', 'fonts' );
			}
		...
		public function schedule_cleanup() {
			if ( ! is_multisite() || ( is_multisite() && is_main_site() ) ) {
				if ( ! wp_next_scheduled( 'pbg_delete_fonts_folder' ) && ! wp_installing() ) {
					wp_schedule_event( time(), self::CLEANUP_FREQUENCY, 'pbg_delete_fonts_folder' );
				}
			}
		}
...
if ( ! function_exists( 'pbg_get_webfont_url' ) ) {
	/**
	 ...
	 */
	function pbg_get_webfont_url( $url, $format = 'woff2' ) {
