...
use Nectar\Utilities\{Log, DemoUtils};
...
class WP_Import extends WP_Importer {
   ...
   */
  function import( $file, $demo_slug ) {
    add_filter( 'import_post_meta_key', [ $this, 'is_valid_meta_key' ] );
    add_filter( 'http_request_timeout', [ &$this, 'bump_request_timeout' ] );

    $started_import = $this->import_start( $file );
    if ($started_import === false) {
      Log::error('Unable to start import of ' . $demo_slug);
      return false;
    }
  ...
  function import_start( $file ) {
    if ( ! is_file( $file ) ) {
      Log::error('The file does not exist, please try again.');
      error_log('The file does not exist, please try again.');
      return false;
    }
  ...
  function import_end($demo_slug) {
    ...
    do_action( 'nectar_demo_import_end', $demo_slug);
  }
  ...
  function get_authors_from_import( $import_data ) {
    if ( ! empty( $import_data['authors'] ) ) {
      ...
      foreach ( $import_data['posts'] as $post ) {
        ...
        if ( empty( $login ) ) {
          error_log(
              sprintf( __( 'Failed to import author %s. Their posts will be attributed to the current user.', 'wordpress-importer' ), esc_html( $post['post_author'] ) )
          );
          continue;
        }
    ...
    foreach ( (array) $_POST['imported_authors'] as $i => $old_login ) {
      ...
      if ( ! empty( $_POST['user_map'][$i] ) ) {
        ...
        if ( ! is_wp_error( $user_id ) ) {
          ...
          error_log(
              sprintf(
                  __( 'Failed to create new user for %s. Their posts will be attributed to the current user.', 'wordpress-importer' ),
  ...
  function process_categories() {
    ...
    foreach ( $this->categories as $cat ) {
      ...
      if ( ! is_wp_error( $id ) && $id > 0 ) {
        ...
        error_log( sprintf( __( 'Failed to import category %s', 'wordpress-importer' ), esc_html( $cat['category_nicename'] ) ) );
        // if ( defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
  ...
  function process_tags() {
    ...
    foreach ( $this->tags as $tag ) {
      ...
      if ( ! is_wp_error( $id ) ) {
        ...
        error_log(
            sprintf( __( 'Failed to import post tag %s', 'wordpress-importer' ), esc_html( $tag['tag_name'] ) )
        );
        // if ( defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
        // 	echo ': ' . $id->get_error_message();
        // }
  ...
  function process_terms() {
    ...
    foreach ( $this->terms as $term ) {
      ...
      if ( ! is_wp_error( $id ) ) {
        ...
        error_log(
            sprintf( __( 'Failed to import %1$s %2$s', 'wordpress-importer' ), esc_html( $term['term_taxonomy'] ), esc_html( $term['term_name'] ) )
        );
        // if ( defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
        // 	echo ': ' . $id->get_error_message();
        // }
      ...
      */
      ...
      if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
        ...
        if ( is_wp_error( $post_id ) ) {
          error_log(
              sprintf(
                  __( 'Failed to import %1$s', 'wordpress-importer' ),
                  $post_type_object->labels->singular_name,
                  esc_html( $post['post_title'] )
              )
          );
        ...
        foreach ( $post['terms'] as $term ) {
          ...
          if ( ! $term_id ) {
            ...
            if ( ! is_wp_error( $t ) ) {
              ...
              error_log(
                  sprintf( __( 'Failed to import %1$s %2$s', 'wordpress-importer' ), esc_html( $taxonomy ), esc_html( $term['name'] ) )
              );
              // if ( defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
      ...
      if ( ! empty( $post['postmeta'] ) ) {
        foreach ( $post['postmeta'] as $meta ) {
          ...
          if ( $key ) {
             // nectar addition
            // export gets meta straight from the DB so could have a serialized string
            if ( ! $value && $key !== '_nectar_blocks_css') {
              $value = maybe_unserialize( $meta['value'] );
            } else {
              $value = $meta['value'];
            }

            // Avoid duplicate single-value meta on re-import: if a meta key already exists for this post,
            // omit adding another row. This keeps importer idempotent for all keys.
  ...
  function process_menu_item( $item ) {
    ...
    if ( ! $menu_slug ) {
      error_log( __( 'Menu item skipped due to missing menu slug', 'wordpress-importer' ));
      return;
    }

    $menu_id = term_exists( $menu_slug, 'nav_menu' );
    if ( ! $menu_id ) {
      error_log( sprintf( __( 'Menu item skipped due to invalid menu slug: %s', 'wordpress-importer' ), esc_html( $menu_slug ) ) );
      return;
    } else {
