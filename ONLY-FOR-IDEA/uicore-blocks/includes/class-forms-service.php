...
class Contact_Form_Service {
    ...
    public function handle() {
        ...
        if (isset($this->form_data['grecaptcha_token']) && !empty($this->form_data['grecaptcha_token'])) {
            ...
            if(!$recaptcha['success']){
                return [
                    'status' => 'error',
                    'data' => [
                        'message' => esc_html__('reCAPTCHA validation failed.', 'uicore-blocks'),
                    ]
                ];
            }
        }
            ...
            foreach ($this->settings['submit_actions'] as $action) {
                try {
                    switch ($action) {
                        ...
                        default:
                            throw new Submit_Exception(esc_html__('Unknown submit action: ', 'uicore-blocks') . $action . esc_html__('. Check your settings.', 'uicore-blocks'));
                    }
        ...
        // There's no need to continue without a submit action enabled
            ...
            return [
                'status' => 'error',
                'data' => [
                    'message' => esc_html__('No submit action enabled.', 'uicore-blocks')
                ]
            ];
        }
        ...
        foreach ($files['form_fields']['tmp_name'] as $input => $value) {
            if ($files['form_fields']['error'][$input] !== UPLOAD_ERR_NO_FILE) {
                ...
                if (!isset($uploaded_file['error'])) {
                    ...
                    $errors = esc_html__('Failed to upload file: ', 'uicore-blocks') . $uploaded_file['error'];
                }
    ...
    protected function mailchimp() {
        ...
        // Get API data
        $key     = get_option('uicore_blocks_mailchimp_secret_key');
    ...
    protected function validate_recaptcha(string $token, string $version) {
        ...
        if (!get_option('uicore_blocks_recaptcha_secret_key') || !get_option('uicore_blocks_recaptcha_site_key')) {
            return [
                'success' => false,
                'message' => esc_html__('reCAPTCHA API keys are not set.', 'uicore-blocks')
            ];
        }

        $data = [
            'secret' => get_option('uicore_blocks_recaptcha_secret_key'),
    ...
    protected function validate_mailchimp(string $key, string $list_id) {
        if (empty($key)) {
            throw new Mailchimp_Exception(esc_html__('Mailchimp API key is not set. Check Uicore Elements settings.', 'uicore-blocks'));
        } else if (empty($list_id)) {
            throw new Mailchimp_Exception(esc_html__('Audience ID control is not set. Check your widget settings.', 'uicore-blocks'));
        }
    }
    ...
    public static function get_default_messages(){
        return [
            // main messages
			'success'    => esc_html__( 'Your submission was successful.', 'uicore-blocks' ),
			'error'      => esc_html__( 'Your submission failed because of an error.', 'uicore-blocks' ),
            'mail_error' => esc_html__( 'Failed to send email.', 'uicore-blocks' ),
            'required'   => esc_html__( 'Fill all required fields.', 'uicore-blocks' ),
            'redirect'   => esc_html__( 'Redirecting...', 'uicore-blocks' ),
		];
    }
    protected function get_response_message(string $status, string $dinamic_data = '') {
        ...
        $default_messages = [
            'invalid_status'    => esc_html__( 'Invalid status message.', 'uicore-blocks' ),
            'redirect_no_url'   => esc_html__( 'Redirection failed. No URL set.', 'uicore-blocks' ),
            'empty_field'       => esc_html__( 'The following field is empty: ', 'uicore-blocks') . $dinamic_data,
        ];
        ...
        if ( isset($responses['mailchimp']) ) {
            ...
            if( is_int($responses['mailchimp']['status'])){
                $data['mailchimp'] = [
                    ...
                    'message' => sprintf( esc_html__('Mailchimp HTTP "%1$s," - "%2$s,."', 'uicore-blocks'), $responses['mailchimp']['status'], $responses['mailchimp']['detail'])
                ];
